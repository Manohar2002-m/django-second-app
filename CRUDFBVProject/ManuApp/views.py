...
from ManuApp.forms import EmployeeForm
from ManuApp.models import Employee

# Create your views here.
def show_view(request):
    employees = Employee.objects.all()
    return render(request, 'ManuApp/index.html', {'employees': employees})
...
def update_view(request,pk):    #use pk only here(same as url)
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('/index')
    return render(request,'ManuApp/update.html',{'employee':employee})
